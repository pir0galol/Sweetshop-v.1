...
namespace cofetarie
{
    public partial class PlatiClienti : MetroFramework.Forms.MetroForm
    {
        ...
        public int id_fact_selectat;
        private int id_metoda_plata;
        public string iban;
        private string rest_plata;
        private bool valid;
        bool exista_plata;
        ...
        void populeazaCuFacturi()
        {
            ...
            string query = "select factura_id from facturi f join comenzi_clienti cc on f.comandac_id = cc.comandac_id";
        ...
        void populareDateFacturaFaraPlata()
        {
            ...
            string query = @"SELECT 	c.client_nume,
	                                    c.iban,
	                                    fact.datascadenta,
	                                    fact.valoarefactura
                            FROM CLIENTI c 	INNER JOIN COMENZI_CLIENTI cc ON c.client_id = cc.client_id
		                                    INNER JOIN FACTURI fact ON fact.comandac_id = cc.comandac_id
            ...
            try
            {
                using (conn = new NpgsqlConnection(conexiune))
                {
                    ...
                    using (NpgsqlDataAdapter adapter = new NpgsqlDataAdapter(query, conn))
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            txtDenumireClient.Text = row["client_nume"].ToString();
        ...
        void populareDateFacturaCuPlata()
        {
            ...
            string query_total = @"SELECT fact.factura_id, 
                                ...
                                FROM PLATI p INNER JOIN METODEPLATI mp ON p.metodaplata_id = mp.metodaplata_id
	                                        ...
	                                        INNER JOIN COMENZI_CLIENTI cc ON cc.comandac_id = fact.comandac_id
	                                        INNER JOIN CLIENTI c ON c.client_id = cc.client_id
                                WHERE fact.factura_id =" + id_fact_selectat;
            try
            {
                using (conn = new NpgsqlConnection(conexiune))
                        ...
                        using (NpgsqlDataAdapter adapter = new NpgsqlDataAdapter(query_total, conn))
                        {
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                if (cboMetodaPlata.SelectedItem.ToString() == "ordin plata")
                                {
                                    ...
                                    txtIBANBeneficiar.Text = "RO99BNR00112233445566778";
                                    txtIBANPlatitor.Text = row["iban"].ToString();
                                }
        ...
        {
            id_fact_selectat = int.Parse(cboNrFactura.Text.ToString());
            string query = @"SELECT SUM(valoare) AS valoaretotala 
                            FROM PLATI 
                            WHERE Factura_Id=" + id_fact_selectat;
            try
            {
                using (conn = new NpgsqlConnection(conexiune))
                {
        ...
        {
            txtIBANBeneficiar.Text = y;
            txtIBANPlatitor.Text = x;
        }
        ...
        void show_informare_integral()
        {
            ...
            lblInformare.Text = "Factura cu nr. " + cboNrFactura.SelectedItem + " a fost platita\nintegral.";
            btnPlatesteRestul.Visible = false;
        }
